...
import { isInstitutionalEmail, isVerifiedInstitutionalUser } from "../auth-policy";

export function assertSessionUser(session) {
  if (!session?.user?.id || !session.user.email || session.user.banned) {
    throw new ApiError(401, "Authentication required");
  }
  if (!isInstitutionalEmail(session.user.email) || !isVerifiedInstitutionalUser(session.user)) {
    throw new ApiError(403, "A verified @vitstudent.ac.in Google account is required");
  }
  return session.user;
}
