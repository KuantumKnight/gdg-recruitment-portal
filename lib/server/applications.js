...
// Every writer for an applicant contends on this same document. Firestore retries
// a transaction when another request updates it before commit.
export async function saveApplication(db, user, data) {
  const email = user.email.trim().toLowerCase();
  const collection = db.collection("formData");
  const indexRef = db.collection("applicationIndexes").doc(identityKey(email));
  return db.runTransaction(async (transaction) => {
    const index = await transaction.get(indexRef);
    let entries = index.exists ? index.data().entries || [] : null;
    if (!entries) {
      const legacy = await transaction.get(collection.where("Email", "==", email));
      const originals = user.email !== email ? await transaction.get(collection.where("Email", "==", user.email)) : null;
      const docs = new Map([...legacy.docs, ...(originals?.docs || [])].map((doc) => [doc.id, doc]));
    ...
    const record = {
      ...
      RegistrationNumber: data.RegistrationNumber, Phone: data.Phone,
    ...
    transaction.set(indexRef, { entries: [...entries, { id: ref.id, department: data.Department }], updatedAt: new Date() });
