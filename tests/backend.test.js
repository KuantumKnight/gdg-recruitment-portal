...
const values = {
  ...
  Gender: "Female",
  "Year of Study": "2",
  motivation: "To learn and build with the community.",
  answers: Object.fromEntries(department.questions.map((q) => [`${department.id}:${q.id}`, `Answer for ${q.id}`])),
};
  ...
  const db = {
    ...
    runTransaction: (callback) => {
      const run = queue.then(async () => {
        const writes = [];
        const transaction = {
          get: async (ref) => ref.path
            ? { exists: records.has(ref.path), data: () => records.get(ref.path) }
            : {
                docs: [...records]
                  .filter(([key, value]) => key.startsWith(`${ref.collection}/`) && value[ref.field] === ref.value)
                  .map(([key, value]) => ({ id: key.split("/")[1], data: () => value })),
              },
          create: (ref, data) => writes.push(() => {
            if (records.has(ref.path)) throw new Error("Duplicate document");
...
describe("response persistence", () => {
  ...
  it("rejects missing required responses and undeclared fields", () => {
    const data = payload();
    delete data.Questions[department.questions[0].name];
  ...
  it("persists full response and takes identity/rank from the server", async () => {
    ...
    expect(db.records.get(`formData/${result.id}`)).toMatchObject({
    ...
    expect(db.records.get(`applicationIndexes/${registrationIdentityKey("25BCE5612")}`)).toMatchObject({
  ...
  it("rejects a different account claiming an indexed registration number", async () => {
    const db = database();
    await saveApplication(db, user, { ...payload(), Department: "one" });
    await expect(saveApplication(db, { id: "user-2", email: "other@example.com" }, { ...payload(), Department: "two" })).rejects.toMatchObject({ status: 409 });
    expect([...db.records.keys()].filter((key) => key.startsWith("formData/"))).toHaveLength(1);
  });

  it("binds a legacy registration number to its existing account", async () => {
    const db = database([{ Email: "other@example.com", RegistrationNumber: "25BCE5612", Department: "legacy-one" }]);
  ...
  it("counts legacy documents when initializing the applicant index", async () => {
    const db = database([
      { Email: user.email, RegistrationNumber: "25BCE5612", Department: "legacy-one" },
      { Email: user.email, RegistrationNumber: "25BCE5612", Department: "legacy-two" },
    ]);
    await expect(saveApplication(db, user, payload())).rejects.toMatchObject({ status: 409 });
  });

  it("normalizes applicant index identity", () => {
    expect(identityKey(" Applicant@Example.com ")).toBe(identityKey("applicant@example.com"));
  });
});
...
describe("request constraints", () => {
  ...
  it("enforces actual body size even without Content-Length", async () => {
    ...
    }), 5)).rejects.toMatchObject({ status: 413 });
  });
});
